$(document).ready(function () {
  ...
  const saveRequestInSessionStorage = (
    ...
    const request = JSON.parse(
      sessionStorage.getItem(requestNameInSessionStorage)
    );
  ...
  // done
  const handleButtonInsertion = async (
    positionElement,
    btnText,
    btnClickHandler,
    active
  ...
  const handleEditFetchedOptionStepValue = async (
    form,
    value,
    module,
    step
  ...
  const handleApplyForLeave = async () => {
    ...
    sessionStorage.setItem(
      leaveRequestModule.moduleKey,
      JSON.stringify(applyForLeaveBody)
    );
