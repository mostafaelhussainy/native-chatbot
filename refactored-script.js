...
const utils = {
  ...
  disableElement: (element) =>
    element.prop("disabled", true).css("color", "#ccc"),
    ...
    return new Promise((resolve, reject) => {
      function type() {
        if (charIndex < textContent.length) {
          targetElement.text(
            targetElement.text() + textContent.charAt(charIndex)
          );
          charIndex++;
...
const messageHandler = {
  ...
  insertAttachment: async (module, step, positionElement, isEdit) => {
    ...
    elements.attachmentInput.change(function () {
      ...
      reader.onload = async (event) => {
        ...
        if (!isEdit) {
          await utils.insertMessage(
            `You've uploaded: ${file.name}`,
            "incoming"
          );
        }
  ...
  insertSummary: async (requestItemName, module) => {
    ...
    const { insertMessage, insertOptionsDiv, insertButton, insertYesOrNo } =
      messageHandler;
    const { getReqFromSessionStorage, disableElement } = utils;
    const { SUBMIT, EDIT, BACK_TO_MENU } = constants;
    ...
    const SUBMIT_REQUEST = await insertMessage(
...
const moduleRequest = {
  start: async () => {
    ...
    const MESSAGE = await insertMessage(
      constants.STARTING_MESSAGE,
      "incoming with-options"
    );
...
const init = () => {
  ...
  const { handleChatBotToggler, handleWidthBtnToggler, chatBotWrapper } =
    interactionHandler;
